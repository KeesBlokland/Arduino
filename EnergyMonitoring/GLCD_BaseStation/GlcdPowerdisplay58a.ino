...
--8 dec: 1 tick is 10 l my friend!  dooohh
--9 dec, removed the write to eeprom in the loop, since I already do it on every packet received.
Added a 100 uF cap on the input power, the usb-wart give funny volts.
At present there are 195 byte free mem. go easy on the print statements when debugging!


ToDo
Make the m3 with a small '3'
Get the tarifs in the right place: Sa Tarif is active. =10K
Remove the A/D code, I'm running on mains now. 
Change code rev to 212.dd  (or 301.dd soon.)
*/
// This is node 13
...
unsigned int T1_kW = 10444;  // T1_kW  inititally put the present meter reading here. Low tarif?
unsigned int T1_watt = 120 ;   // the x,000 bit that you can't see, but want to save anyway between restarts.
unsigned int T2_kW = 6367;   // T2_kW inititally put the present meter reading here.
unsigned int T2_watt = 841 ;
unsigned int TotalGas = 53565 ;   // 
int inc_gas_pulse      = 550;  // place for emongas. TotalGasPulse read at least 2  digits and enter xx0 here. 
boolean OverRide   =0;  // override EEprom values, force writing these.
boolean Weekend    = 0;  // Is it weekend? check now[3]. 7=su 6=sa.
...
//const int UNO = 1;   // always for Jeelnodes
//--------------------------
...
void WriteGLCDHeader () {
             ...
             glcd.drawString(0,8, "monitor. Rev 212.09A");  // change rev to show date. 
             }
...
void loop () {    //main loop
       ...
       //--------Write everything to the screen-------
          ...
          ++z ;  // used by jeenode to see if things happen..
        //  Serial.println (z);
          if (z > 10 ){
...
void IncrementGas () {
            inc_gas_pulse = inc_gas_pulse + 10 ;  // every tick is 1 rev = 10 Liter.
            if (inc_gas_pulse >= 1000)  {
               TotalGas = TotalGas + 1 ;  // 1000 liters = 1m3
