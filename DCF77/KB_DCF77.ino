...
void loop() {
    if (dcfPoll(DCF77.digiRead())) {
         //Serial.print("DCF77 ");   //optional, keep it shorT
        // char buf[30];
        ...
        //                            hour, minute, 0, 0));
       // Serial.println(buf);
      //  Serial.print((int) dst);  // Daylight Saving Time.
      //  Serial.print(' ');
        if (1) {
            // shortest format? hh:mm dd-mm-yyyy dst  since I usually only care about the hhmm
            // watch, this code (did) does not add leading zero's to hrs/min!
        ...
        // handy if you need this for calculations.
        // Serial.println(unixTime(date2days(year, month, day), hour, minute, 0, 0));
